...
This is a tool to run and interact with a local Axia network.
...
```sh
git clone https://github.com/axiacoin/axia-network-runner.git
```
...
The E2E test checks `axia-network-runner` RPC communication and control. It starts a network against a fresh RPC
server and executes a set of query and control operations on it.

To start it, execute inside the cloned directory:

```sh
./scripts/tests.e2e.sh AXIA_VERSION1 AXIA_VERSION2
```
...
## Using `axia-network-runner`

You can import this repository as a library in your Go program, but we recommend running `axia-network-runner` as a binary.
This creates an RPC server that you can send requests to in order to start a network, add nodes to the network, remove nodes from the network, restart nodes, etc.. You can make requests through the `axia-network-runner` command or by making API calls. Requests are "translated" into gRPC and sent to the server.

**Why does `axia-network-runner` need an RPC server?** `axia-network-runner` needs to provide complex workflows such as replacing nodes, restarting nodes, injecting fail points, etc.. The RPC server exposes basic operations to enable a separation of concerns such that one team develops a test framework, and the other writes test cases and controlling logic.
...
Download from https://github.com/axiacoin/axia-network-runner/releases:

```bash
# or install
cd ${HOME}/go/src/github.com/axiacoin/axia-network-runner
...
To start a new Axia network with five nodes (a cluster):
...
The network-runner supports axia node configuration at different levels.
1. If neither `--global-node-config` nor `--custom-node-configs` is supplied, all nodes get a standard set of config options. Currently this set contains:
    ...
    ```
2. `--global-node-config` is a JSON string representing a *single* axia config, which will be applied to **all nodes**. This makes it easy to define common properties to all nodes. Whatever is set here will be *combined* with the standard set above.
...
# or
axia-network-runner control save-snapshot snapshotName
```
...
# or
axia-network-runner control load-snapshot snapshotName
```
...
# or
axia-network-runner control get-snapshot-names
```
...
# or
axia-network-runner control remove-snapshot snapshotName
```
...
curl -X POST -k http://localhost:8081/v1/control/restartnode -d '{"name":"node1","execPath":"'${AXIA_EXEC_PATH}'","logLevel":"INFO"}'
...
# a different execPath
curl -X POST -k http://localhost:8081/v1/control/addnode -d '{"name":"node99","execPath":"'${AXIA_EXEC_PATH}'","logLevel":"INFO"}'

# or
axia-network-runner control add-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node99 \
...
`--node-config` allows to specify specific axia config parameters to the new node.
See [here](https://docs.axc.network/build/references/axia-config-flags) for the reference of supported flags.
...
# or
axia-network-runner control attach-peer \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node1
...
# or
axia-network-runner control send-outbound-message \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node1 \
...
```bash
# or download from https://github.com/axiacoin/subnet-cli/releases
cd ${HOME}/go/src/github.com/axiacoin/subnet-cli
go install -v .
...
// SwapChain Address 1:     X-custom18jma8ppw3nhx5r4ap8clazz0dps7rv5u9xde7p
// SwapChain Address 1 Key: PrivateKey-ewoqjP7PxY4yr3iLTpLisriqt94hdyDFNgchSxGGztUrTXtNN
// SwapChain Address 2:     X-custom16045mxr3s2cjycqe2xfluk304xv3ezhkhsvkpr
// SwapChain Address 2 Key: PrivateKey-2fzYBh3bbWemKxQmMfX6DSuL2BFmDSLQWTvma57xwjQjtf8gFq
// CoreChain Address 1:     P-custom18jma8ppw3nhx5r4ap8clazz0dps7rv5u9xde7p
// CoreChain Address 1 Key: PrivateKey-ewoqjP7PxY4yr3iLTpLisriqt94hdyDFNgchSxGGztUrTXtNN
// CoreChain Address 2:     P-custom16045mxr3s2cjycqe2xfluk304xv3ezhkhsvkpr
// CoreChain Address 2 Key: PrivateKey-2fzYBh3bbWemKxQmMfX6DSuL2BFmDSLQWTvma57xwjQjtf8gFq
// AXChain Address:       0x8db97C7cEcE249c2b98bDC0226Cc4C2A57BF52FC
// AXChain Address Key:   56289e99c94b6912bfc12adc093c9b51124f0dc54ac7a766b2bc5ccf558d8027
